...
void main(){
  ...
  uv = vec2(uv.x+ cnoise(rote * 10.), (uv.y+ snoise(roteC * 15.)));
  ...
  uvRipple(uv, .09);
    ...
    // color.r = stroke(snoise(uv1 * 20. * cnoise(uv1 *4.) * uv1.y ), .4, .2) ;
    // color.b = stroke(snoise(uv1 * 2. * cnoise(uv *1.) * uv.y ), .4, .2) ;

    color += mix(color, 1.-color,stroke(polySDF(uv, 20.), .6, .2));
  //
    color += mix(color, vec3(0., uv.y, uv.x),stroke(polySDF(uv, 3.), .5, .5));

    color += mix(color, vec3(uv.y, 1., 1.),stroke(polySDF(uv, 30.), .5, .5));
  if(color == vec3(0.)){
    color += snoise(uv1 * 1. * cnoise(uv1 * 4.) * uv.y);
    uvRipple(color.rg, .6);
    brownConradyDistortion(color.rb, -1., 1. * sin(vTime));
  }
  //
  // color += stroke(cnoise( uv2 *4. * cnoise(rote * 40.) * uv.y), .2, .2);
